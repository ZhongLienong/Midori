...
| `close`, `is_done`, `cancel` | **moved** | 2026-09-16. They were unqualified builtins while the rest of the concurrency surface moved under `Concurrency::`; they are now `Concurrency::Close`, `IsDone` and `Cancel`, on the same intrinsic path, and the old spellings name their replacement. |
