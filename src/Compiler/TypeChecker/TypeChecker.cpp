...
MidoriResult::TypeResult TypeChecker::Unify(const Token& token, std::shared_ptr<MidoriType>& left, std::shared_ptr<MidoriType>& right)
{
	...
	else if (left_subst->IsType<MidoriType::TypeVariable>())
	{
		...
		{
			return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Infinite type detected (occurs check failed): ", token, left_subst, right_subst));
		}
	...
	else if (left_subst->IsType<MidoriType::FunctionType>() && right_subst->IsType<MidoriType::FunctionType>())
	{
		...
		{
			return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Unable to unify: ", token, left_subst, right_subst));
		}
	...
	else if (left_subst->IsType<MidoriType::UnionType>() && right_subst->IsType<MidoriType::UnionType>())
	{
		...
		{
			auto right_it = right_union.m_member_info.find(member_name);
			if (right_it == right_union.m_member_info.end())
			{
				return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Unable to unify: ", token, left_subst, right_subst));
			}

			const auto& right_ctx = right_it->second;
...
std::shared_ptr<MidoriType> TypeChecker::Freshen(const std::shared_ptr<MidoriType>& type, FresheningContext& context)
{
	...
	auto cache_it = context.type_cache.find(type.get());
	if (cache_it != context.type_cache.end())
	{
	...
	else if (type->IsType<MidoriType::GenericParam>())
	{
		...
		auto it = context.generic_params.find(param_name);
		if (it != context.generic_params.end())
		{
		...
		else
		{
			...
			context.generic_params[param_name] = fresh_var;
			return fresh_var;
		}
	}
	...
	else if (type->IsType<MidoriType::FunctionType>())
	{
		...
		{
			fresh_params.push_back(Freshen(param_type, context));
		}
	...
	else if (type->IsType<MidoriType::StructType>())
	{
		...
		context.type_cache[type.get()] = fresh_struct;
	...
	else if (type->IsType<MidoriType::UnionType>())
	{
		...
		context.type_cache[type.get()] = fresh_union;
...
{
	// Check cache first to handle recursive types
	auto cache_it = cache.find(type.get());
...
TypeChecker::TypeChecker(MidoriProgramTree&& parser_result)
	: m_program_tree(std::move(parser_result)), m_next_type_var_id(0)
{
}
...
MidoriResult::TypeResult TypeChecker::operator()(MidoriStatement::Define& def)
{
	...
	if (def.m_value->IsExpression<MidoriExpression::Function>())
	{
		...
		if (def.m_annotated_type.has_value())
		{
			...
			{
				return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Define statement type error: function type annotation doesn't match function signature", def.m_name, function_type, annotated_type));
			}
		}
	...
	// General case - handles Construct, Array, Loop, and all other expressions uniformly
	return std::visit([this](auto&& arg) { return (*this)(arg); }, **def.m_value)
		.and_then
		(
			[&def, this](std::shared_ptr<MidoriType>&& type)->MidoriResult::TypeResult
			{
				...
				if (type->IsType<MidoriType::ArrayType>())
				{
					...
					{
						return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Define statement type error: empty arrays require type annotations. Use 'def " + def.m_name.m_lexeme + " : Array<ElementType> = [];'", def.m_name.m_line));
					}
				}
...
MidoriResult::TypeResult TypeChecker::operator()(MidoriStatement::DefineFunction& defun)
{
	...
	if (!defun.m_generic_params.empty())
	{
		...
		for (const Token& generic_param : defun.m_generic_params)
		{
			...
			{
				return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("DefineFunction type error: duplicate generic parameter name", generic_param.m_line));
			}
		}
		...
		for (const Token& param : defun.m_params)
		{
			...
			{
				return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("DefineFunction type error: generic parameter conflicts with function parameter", param.m_line));
			}
		}
	}
	...
	// Save a copy of return type before creating function type, since MakeFunctionType moves it
	...
	// Track generic functions so they can be freshened at each call site
	if (!defun.m_generic_params.empty())
	{
	...
	// Add generic type parameters to the type environment as fresh type variables
	for (const Token& generic_param : defun.m_generic_params)
	{
	...
	(
		...
		[&defun, this](size_t idx) {m_name_type_table.back().emplace(defun.m_params[idx].m_lexeme, defun.m_param_types[idx]); }
	);

	// Set the expected return type so that return statements can validate against it
	...
	return std::visit([this](auto&& arg) { return (*this)(arg); }, **defun.m_body)
		...
		(
			...
			{
				EndScope();
				m_expected_return_type = saved_expected_return_type;  // Restore
...
{
	// If struct has generic parameters, validate them
	if (!struct_stmt.m_generic_params.empty())
	{
		...
		for (const Token& generic_param : struct_stmt.m_generic_params)
		{
			if (!generic_param_names.insert(generic_param.m_lexeme).second)
			{
				return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Struct declaration type error: duplicate generic parameter name", generic_param.m_line));
			}
		}

		// Track this as a generic struct
...
{
	// If union has generic parameters, validate them
	if (!union_stmt.m_generic_params.empty())
	{
		...
		for (const Token& generic_param : union_stmt.m_generic_params)
		{
			if (!generic_param_names.insert(generic_param.m_lexeme).second)
			{
				return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Union declaration type error: duplicate generic parameter name", generic_param.m_line));
			}
		}

		// Track this as a generic union
	...
	{
		// Copy member_types instead of moving to preserve them for pattern matching
								...
								(
									[&binary, &left_type, &right_type, this](std::shared_ptr<MidoriType>&&)->MidoriResult::TypeResult
									{
										...
										if (std::ranges::contains(m_binary_partial_order_comparison_operators.cbegin(), m_binary_partial_order_comparison_operators.cend(), binary.m_op.m_token_name))
										{
											...
											{
												return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Binary expression type error", binary.m_op, resolved_self, MidoriType::MakeLiteralType<MidoriType::IntegerType>(), MidoriType::MakeLiteralType<MidoriType::FloatType>()));
											}
										...
										else if (std::ranges::contains(m_binary_bitwise_operators.cbegin(), m_binary_bitwise_operators.cend(), binary.m_op.m_token_name))
										{
											...
											{
												return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Binary expression type error", binary.m_op, resolved_self, MidoriType::MakeLiteralType<MidoriType::IntegerType>()));
											}
										}
										...
										else if (std::ranges::contains(m_binary_logical_operators.cbegin(), m_binary_logical_operators.cend(), binary.m_op.m_token_name))
										{
											...
											{
												return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Binary expression type error", binary.m_op, resolved_left_logical, MidoriType::MakeLiteralType<MidoriType::BoolType>()));
											}
										...
										else if (std::ranges::contains(m_binary_concatenation_operators.cbegin(), m_binary_concatenation_operators.cend(), binary.m_op.m_token_name))
										{
											...
											{
												return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Binary expression type error: expected array or text", binary.m_op, resolved_left));
											}
										}
...
MidoriResult::TypeResult TypeChecker::operator()(MidoriExpression::Call& call)
{
	return std::visit([this](auto&& arg) { return (*this)(arg); }, **call.m_callee)
		...
		(
			...
			{
				// Apply substitution to resolve any unified types
...
MidoriResult::TypeResult TypeChecker::operator()(MidoriExpression::BoundedName& variable)
{
	for (TypeChecker::TypeEnvironmentStack::reverse_iterator it = m_name_type_table.rbegin(); it != m_name_type_table.rend(); ++it)
	{
		...
		if (var != it->end())
		{
			// Check if this is a generic function - if so, freshen its type for each use
...
MidoriResult::TypeResult TypeChecker::operator()(MidoriExpression::Function& function)
{
	...
	{
		return std::unexpected<std::string>(MidoriError::GenerateTypeCheckerError("Function expression type error: lambda expressions cannot have generic parameters. Use 'defun' instead.", function.m_function_keyword.m_line));
	}
...
MidoriResult::TypeResult TypeChecker::operator()(MidoriExpression::Construct& construct)
{
	...
	for (TypeChecker::TypeEnvironmentStack::const_reverse_iterator it = m_name_type_table.crbegin(); it != m_name_type_table.crend(); ++it)
	{
		...
		if (var != env.end())
		{
			// Check if this is a generic struct/union - if so, freshen its type
...
MidoriResult::TypeResult TypeChecker::operator()(MidoriExpression::Array& array)
{
	...
	{
		// Empty arrays get undecided element type, will be inferred from usage
